...
### Version History
- **1.14.2** (2026-01-03): Fixed scroll position reset when marking episode as watched. Changed the detail modal update logic to only update the pill selector's active states instead of re-rendering the entire modal. This preserves the user's scroll position in the episodes list while still properly updating the status indicator from "to watch" to "watching".
- **1.14.1** (2026-01-03): Fixed two issues with TV show status updates. (1) Detail modal now refreshes after marking an episode as watched, properly updating the pill selector to show "watching" status instead of remaining on "to watch". (2) Clicking the "watching" button for a TV show that has no episodes watched now automatically marks the first episode (S01E01) as watched, ensuring the status properly changes to "watching". Previously the watching button had no effect because watching status requires at least one episode to be marked as watched.
